...
public class ThirdPersonPlayer_custom : MonoBehaviour
{
    ...
    public void Move(Vector3 move, bool jump)
    {
        ...
        else
        {
            ...
            //Debug.Log("Move" + moveWithoutProceed);
        }
    ...
    void UpdateAnimator(Vector3 move)
    {
        ...
        if (m_IsGrounded && move.magnitude > 0)
        {
            ...
            Vector3 moveDir = transform.InverseTransformDirection(m_Rigidbody.velocity + move) ;
            m_Rigidbody.velocity = new Vector3(-moveDir.x * m_MoveSpeedMultiplier,
                                               moveDir.y,
                                               moveDir.z * m_MoveSpeedMultiplier) ;
        }
        else
        {
    ...
    void CheckGroundStatus()
    {
        ...
        Vector3 capsul_center = GroundCheckCollider.transform.position;
        bool isGround_collider = Physics.CheckSphere(capsul_center , m_GroundCheckDistance, groundMask, QueryTriggerInteraction.UseGlobal);
        bool isGround_ray = Physics.Raycast(transform.position + (Vector3.up * 0.1f), Vector3.down, out hitInfo, m_GroundCheckDistance);
        //Debug.DrawRay(transform.position + (Vector3.up * 0.1f), Vector3.down,Color.red);
